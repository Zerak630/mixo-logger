...
public class AuthentificationTests : IClassFixture<AuthentificationTests.ApiDeTest>
{
    ...
    private static Task<HttpResponseMessage> SeConnecter(HttpClient client, string identifiant = Identifiant, string motDePasse = MotDePasse) =>
        client.PostAsJsonAsync("/api/auth/connexion", new { identifiant, motDePasse });

    [Fact]
    public async Task SansSession_LApiRepond401()
    {
        var reponse = await NouveauClient().GetAsync("/api/cocktails");
    ...
    {
        Assert.Equal(HttpStatusCode.Unauthorized, (await NouveauClient().GetAsync(url)).StatusCode);
    }

    [Fact]
    public async Task SansSession_LEcritureEstAussiRefusee()
    {
        var reponse = await NouveauClient().PostAsJsonAsync("/api/bars/ingredients", new { name = "Gin" });
    ...
    {
        Assert.Equal(HttpStatusCode.OK, (await NouveauClient().GetAsync("/ping")).StatusCode);
    }
    ...
    public async Task AvecSession_LApiRepond_EtMoiRenvoieLUtilisateur()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/api/cocktails")).StatusCode);

        var moi = await client.GetFromJsonAsync<Dictionary<string, object>>("/api/auth/moi");
    ...
    {
        var premier = await (await SeConnecter(NouveauClient())).Content.ReadFromJsonAsync<Dictionary<string, object>>();
        var second = await (await SeConnecter(NouveauClient())).Content.ReadFromJsonAsync<Dictionary<string, object>>();
    ...
    public async Task Deconnexion_FermeLaSession()
    {
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await client.PostAsync("/api/auth/deconnexion", null)).StatusCode);
    ...
    {
        Assert.Equal(HttpStatusCode.NoContent, (await NouveauClient().PostAsync("/api/auth/deconnexion", null)).StatusCode);
    }
    ...
    public async Task Cors_AutoriseLeFrontAvecCookies()
    {
        ...
        var reponse = await NouveauClient().SendAsync(preflight);
    ...
    private static async Task<string?> Detail(HttpResponseMessage reponse) =>
        (await reponse.Content.ReadFromJsonAsync<Dictionary<string, object>>())?["detail"]?.ToString();
}
    ...
    public async Task AuDelaDeLaLimite_LaConnexionEstRefuseeEn429()
    {
        ...
        for (int i = 0; i < 3; i++)
            Assert.Equal(HttpStatusCode.Unauthorized,
                (await client.PostAsJsonAsync("/api/auth/connexion", new { identifiant = "alice", motDePasse = "faux-mot-de-passe" })).StatusCode);

        var bloquee = await client.PostAsJsonAsync("/api/auth/connexion", new { identifiant = "alice", motDePasse = "mot-de-passe-de-test-uniquement" });
